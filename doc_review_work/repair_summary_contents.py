...
def level(p):
    ...
    if style is None:
        return None
    value = style.get(qn(W, "val"), "").lower()
    return {"heading1": 1, "heading2": 2, "heading3": 3}.get(value)
...
def main(source, destination):
    ...
    toc = next(p for p in paras if p.xpath(".//w:bookmarkStart[@w:name='TOC_main']", namespaces=NS))
    toc_i = children.index(toc)
    section1 = next(p for p in children[toc_i + 1:] if level(p) == 1 and text(p).startswith("1. Requirements"))
    section1_i = children.index(section1)
    toc_block = children[toc_i + 1:section1_i]
    l1_template = next(p for p in toc_block if text(p).startswith("Introduction:"))
    ...
    before_toc = paras[:paras.index(toc)]
    ...
    # Main body: sections 1–15 and their Heading 2 subsections, stopping at Appendix C.
    ...
    # Preserve the manual's deliberately compact appendix navigation.
    appendix_specs = [
        ...
        (1, "Appendix A", None),
        (2, "a. Version tag", None),
        (2, "b. Per-device backup", None),
        (1, "Appendix B", None),
        (1, "Appendix D", None),
        (1, "Appendix E", None),
        (1, "Appendix F", None),
        ...
        (2, "e. Profile", None),
        (2, "f. Sleeping", None),
        (2, "g. Removed 2026-08-22", None),
        (1, "Appendix G", None),
    ]
    for lev, prefix, display in appendix_specs:
        target_level = None if prefix in (
    ...
    for lev, label, p in selected:
        anchor = bookmark(p)
        if not anchor:
            anchor = "appendix_g_live_settings" if label.startswith("Appendix G") else f"toc_summary_{generated:03d}"
            generated += 1
    ...
    # Repair screenshot links by making each named destination land on the retained Screenshots page.
    image_aliases = {
        link.get(qn(W, "anchor"))
        for link in root.xpath(".//w:hyperlink[@w:anchor]", namespaces=NS)
        if link.get(qn(W, "anchor"), "").startswith("img_")
    }
    ...
    for child in toc_block:
        body.remove(child)
    insert_at = list(body).index(toc) + 1
