...
// Single live annotation point: Libre 1-minute delta only, attached directly to the current Libre
// graph point (at its actual x/y position) rather than a fixed row, so it stands out right where the
// eye already is, in yellow. yValue must be the same current BG value (display units) as the actual
// plotted glucose point, or the label lands at the wrong height.
class L1DeltaDataPoint(
    ...
    override val shape = Shape.L1_DELTA_POINT
