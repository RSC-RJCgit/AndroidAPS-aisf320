            ...
            if (!overdraw) {
                if (value.shape == Shape.BG || value.shape == Shape.COB_FAIL_OVER) {
                    ...
                    if (value.label.isNotEmpty()) {
                        ...
                        mPaint.style = Paint.Style.FILL
                        mPaint.textAlign = Paint.Align.LEFT
                        // Same fixed glucose-scale anchor as graph5's pp/acc/du row, but one line below
                        // it: this places the text at the top of the basal-column area without tracking
                        // live BG or colliding with the existing row above.
