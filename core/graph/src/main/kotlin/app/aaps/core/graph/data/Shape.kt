...
enum class Shape {
    ...
    SMB_STACK_DELTA_IOB // One label per reconstructed SMB-stack start (same <=70s/10-min rule as ApsAutoIsfSmbStackStart in DetermineBasalAutoISF.kt), drawn at that stack's own start timestamp, fixed near the TOP of the IOB/COB panel (wherever the user has that assigned via the long-press chart menu -- currently g==0, NOT the same panel as SMB_GRAPH2's own SMB dose labels) always in white -- shows the IOB change from the stack's start to 10 minutes later ("+0.42" etc). Stacks downward in 30-min groups when two labels land close together. See PrepareTreatmentsDataWorker.kt for the reconstruction.
}
