...
enum class Shape {
    ...
    SMB_STACK_DELTA_IOB // One label per reconstructed SMB-stack start (same <=70s/10-min rule as ApsAutoIsfSmbStackStart in DetermineBasalAutoISF.kt), drawn at that stack's own start timestamp, fixed near the TOP of graph2 (IOB graph) always in white -- shows the IOB change from the stack's start to 10 minutes later ("+0.42" etc). See PrepareTreatmentsDataWorker.kt for the reconstruction.
}
