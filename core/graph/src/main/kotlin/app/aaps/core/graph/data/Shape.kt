...
enum class Shape {
    ...
    SMB_STACK_DELTA_IOB // One label per reconstructed SMB-stack start (same <=70s/10-min rule as ApsAutoIsfSmbStackStart in DetermineBasalAutoISF.kt), drawn at that stack's own start timestamp, anchored in the top half of the IOB/COB panel (same spot as NOTE_ARROWHEAD_GRAPH3's arrows, 0.2 of the panel's own height down from its top -- wherever the user has that panel assigned via the long-press chart menu, currently g==0, NOT the same panel as SMB_GRAPH2's own SMB dose labels) always in white -- shows the IOB change from the stack's start to 10 minutes later ("+0.42" etc). Stacks UPWARD from that anchor, toward the top edge, in 30-min groups when two labels land close together. See PrepareTreatmentsDataWorker.kt for the reconstruction.
}
