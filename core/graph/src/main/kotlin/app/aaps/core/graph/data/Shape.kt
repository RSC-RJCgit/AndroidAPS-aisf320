...
enum class Shape {
    ...
    IOB_PEAK_MAIN_BOTTOM, // red bold copy of an IOB peak label, fixed at its timestamp along the bottom of the main graph's basal-column area
    ...
    TARGET_OFFSET_DUT_GRAPH5,
    TARGET_OFFSET_DUT_GRAPH1,
    ...
    PP_ACC_DU_ROW, // "pp= acc= du=" row, graph5 only. Positioned via the point's own real Y (endY, like any normal data point) rather than a graphHeight pixel fraction -- graph5's basal bars occupy negative Y below the glucose floor (same structure as bg_graph; an earlier assumption that graph5 had no basal region was wrong -- graph5Data.addBasals() IS called there for tempBasal-capable pumps), so a pixel fraction lands in that zone instead of near actual BGL values. The caller feeds a FIXED display-unit value near 2.8 mmol (not the live current BG; lowered 2026-08-15 from 4.0 mmol, which sat too close to/inside the in-range green BG trace) so this sits clearly below any in-range BG point regardless of where BG currently is. Added to yIndependentShape (PointsWithLabelGraphSeries.kt) the same day, after the lower anchor started getting culled whenever it fell below the graph's currently visible Y-range.
    ...
    // Exactly 12 white totals on the IOB graph (graph2), top half, one per equal slice of the visible
    // fromTime..toTime window (so 5 min on 1h, 15 min on 3h, 30 min on 6h, 1h on 12h, 1.5h on 18h,
    // 2h on 24h). Each number is insulin delivered in that slice: SMB + meal/manual bolus + basal
    // (temp basal absolute; extended bolus only when the pump is not faking TBR with EB).
    INSULIN_INTERVAL_TOTAL
}
