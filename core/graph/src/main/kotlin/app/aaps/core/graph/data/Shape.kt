...
enum class Shape {
    ...
    PP_ACC_DU_ROW, // "pp= acc= du=" row, graph5 only. Positioned via the point's own real Y (endY, like any normal data point) rather than a graphHeight pixel fraction -- graph5's basal bars occupy negative Y below the glucose floor (same structure as bg_graph; an earlier assumption that graph5 had no basal region was wrong -- graph5Data.addBasals() IS called there for tempBasal-capable pumps), so a pixel fraction lands in that zone instead of near actual BGL values. The caller feeds a FIXED display-unit value near 3.3 mmol (not the live current BG; lowered 2026-08-15 from 4.0 mmol, which sat too close to/inside the in-range green BG trace) so this sits clearly below any in-range BG point regardless of where BG currently is.
