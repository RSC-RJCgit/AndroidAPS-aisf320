...
enum class Shape {
    ...
    PP_ACC_DU_ROW, // "pp= acc= du=" row, main graph only. Positioned relative to the point's OWN real Y (current BG, in display units), NOT a fixed fraction of graph height -- that landed in the main graph's basal-column area at the bottom (same zone as HP_ROW_BOTTOM), not the green BGL-plotted area where SMB triangles/arrows actually sit. A small offset below the current BG point approximates "just above the level of a current SMB triangle" (itself anchored to its own dose's BG point, not a fixed row, so this can only ever be an approximation).
