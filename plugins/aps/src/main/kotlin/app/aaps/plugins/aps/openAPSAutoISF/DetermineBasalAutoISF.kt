...
class DetermineBasalAutoISF @Inject constructor(
    ...
    // Was: Tier 3's ORIGINAL gate shape (delta/ratio thresholds, throttle, quiet window, sub-7.5mmol
    // guard, including eventualBG/insulinReq) with acce_ISF swapped in for bg_acce, kept purely to log
    // how often it WOULD have fired, independent of bmildBasicCriteriaMet. Never had any dosing effect.
    // acceIsfValue (the determine_basal() parameter this fed) is left in place -- also used by the
    ...
    fun
        ...
        if (iob_data.iob > max_iob) {
            ...
            if (microBolusAllowed && enableSMB && bg > threshold) {
                ...
                // (see BooleanKey.ApsAutoIsfUamBoostEnabled / DoubleKey.ApsAutoIsfUamBoost* and their
                // own doc comments) instead of hardcoded local literals. boost_scale is additionally
                ...
                // Tier 3 is intentionally daytime/evening only, except a brand-new pod (<2h) already
                // over 9.0 mmol -- that bypass is computed in OpenAPSAutoISFPlugin (same helper as
                // BMild/bg3 day windows) and passed in so replayed cycles keep the live decision.
                // Use the calculation's systemTime so replayed/historical calculations are evaluated
                // in their own local-time window too, rather than against the wall clock at the moment
                ...
                if (boostActive && tier3TimeAllowed) {
                    ...
                    // T3AcceISF observation block removed 2026-08-27 per explicit instruction (no longer
                    // wanted) -- see acceIsfValue's own doc comment above for what it used to feed.
