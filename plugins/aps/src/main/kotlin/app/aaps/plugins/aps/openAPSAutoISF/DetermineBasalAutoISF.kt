...
class DetermineBasalAutoISF @Inject constructor(
    ...
    fun determine_basal(
        ...
        if (autoIsfMode) {
            consoleError.add("----------------------------------")
            consoleError.add("start AutoISF ${profile.autoISF_version} __ 320.009")
            consoleError.add("----------------------------------")
        ...
        rT.reason.append(
            " 320.009 COB: ${round(meal_data.mealCOB, 1).withoutZeros()}, Dev: ${convert_bg(deviation.toDouble())}, BGI: ${convert_bg(bgi)}, ISF: ${convert_bg(sens)}, CR: ${
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        if (iob_data.iob > max_iob) {
            ...
            if (microBolusAllowed && enableSMB && bg > threshold) {
                ...
                consoleError.add("offsetSoZeroSMB $offsetSoZeroSMB")
                var LibreTrue = 1.00
                val lower_SMB = profile.smb_delivery_ratio_min
                ...
                //microBolus > 0.3 //not used except rsn049
                    ...
                    if (Delta >= 1.0 * 18 &&
                        ...
                        if (bg > 8.8 * 18) {
                            ...
                            ( microBolus > lower_SMB * 0.030 * max_iob ||
                                nowHour <= 8 )
                        ){
                            microBolus = microBolus * 0.6
                            rT.reason.append("microBolus ov lower_SMB * 0.030 * max_iob  = microBolus * 0.6 ; microBolus = ${microBolus} ")
                            rT.reason.append(" CHANGED SIZE 0.604 for moderate fast rise 0.604 ")
                        } else if (bg <= 8.0 * 18 && microBolus <= lower_SMB * 0.030 * max_iob ){
                        ...
                        if (bg > 8.8 * 18) {
                            microBolus = microBolus * 0.8
                            rT.reason.append("microBolus = microBolus * 0.8 ; microBolus = ${microBolus} ")
                            rT.reason.append(" CHANGED SIZE 0.805 for mild fast rise 0.905 ")
                        } else if (bg > 8.0 * 18) {
                            microBolus = microBolus * 0.7
                            rT.reason.append("microBolus = microBolus * 0.7 ; microBolus = ${microBolus} ")
                            ...
                            microBolus = microBolus * 0.5
                            rT.reason.append("microBolus ov lower_SMB *  0.030 * max_iob  = microBolus * 0.5 ; microBolus = ${microBolus} ")
                    ...
                    if ( microBolus > lower_SMB * 0.030 * max_iob ||
                        nowHour <= 8
                    ) {
                        microBolus = microBolus * 0.6
                        rT.reason.append("microBolus ov lower_SMB *  0.030 * max_iob = microBolus * 0.6 ; microBolus = ${microBolus} ")
...
DetermineBasalAutoISF.kt 320.009
 */