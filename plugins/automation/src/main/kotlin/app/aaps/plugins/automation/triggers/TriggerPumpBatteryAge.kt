...
class TriggerPumpBatteryAge(injector: HasAndroidInjector) : Trigger(injector) {
    ...
    override fun shouldRun(): Boolean {
        ...
        if (comparator.value.check(currentAgeHours, pumpBatteryAgeHours.value, 0.001)) {
            aapsLogger.debug(LTag.AUTOMATION, "Ready for execution: " + friendlyDescription())
            return true
        }
        aapsLogger.debug(LTag.AUTOMATION, "NOT ready for execution: " + friendlyDescription())
    ...
    override fun friendlyDescription(): String =
        rh.gs(R.string.triggerPumpBatteryAgeDesc, comparator.value.shortSymbol, pumpBatteryAgeHours.value)
