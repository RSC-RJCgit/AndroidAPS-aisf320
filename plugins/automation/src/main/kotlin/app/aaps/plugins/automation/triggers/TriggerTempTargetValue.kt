...
class TriggerTempTargetValue(injector: HasAndroidInjector) : Trigger(injector) {
    ...
    override fun shouldRun(): Boolean {
        ...
        if (tt != null && comparator.value.check(tt.lowTarget, profileUtil.convertToMgdl(ttValue.value, ttValue.units), 0.001)) {
            aapsLogger.debug(LTag.AUTOMATION, "Ready for execution: " + friendlyDescription())
            return true
        }
    ...
    override fun friendlyDescription(): String {
        return if (comparator.value == Comparator.Compare.IS_NOT_AVAILABLE)
            ...
            rh.gs(if (ttValue.units == GlucoseUnit.MGDL) R.string.temptargetcomparedmgdl else R.string.temptargetcomparedmmol, comparator.value.shortSymbol, ttValue.value, ttValue.units)
    }
