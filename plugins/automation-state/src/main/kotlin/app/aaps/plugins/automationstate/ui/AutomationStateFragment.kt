...
class AutomationStateFragment : DaggerFragment(), MenuProvider {
    ...
    companion object { const val ID_MENU_ADD_STATE = 601 }
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean =
        when (item.itemId) {
            ID_MENU_ADD_STATE -> { AutomationAddStateDialog.newInstance().show(childFragmentManager, "AutomationAddStateDialog"); true }
            else -> false
        }
    ...
    inner class StateAdapter : RecyclerView.Adapter<StateAdapter.StateViewHolder>() {
        ...
        fun updateStates() {
            ...
            states.forEach { (stateName, _) -> stateValues[stateName] = automationStateService.getStateValues(stateName) }
        ...
        override fun onBindViewHolder(holder: StateViewHolder, position: Int) {
            ...
            val screenWidth = resources.displayMetrics.widthPixels - 48
            stateValues[stateName]?.forEach { stateValue ->
                ...
                val stateViewWidth = stateView.measuredWidth + 16
                ...
                val params = LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply { setMargins(0, 0, 8, 0) }
                currentRow.addView(stateView, params)
                currentRowWidth += stateViewWidth
            }
            holder.itemView.setOnClickListener {
                AutomationStateValuesDialog.newInstance(stateName).show(childFragmentManager, "AutomationStateValuesDialog")
            }
        }
