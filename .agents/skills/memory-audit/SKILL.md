...
When spawning agents, repeat this repo's Bash rules in the prompt - agents do not follow AGENTS.md
...
checkout under `.Codex/worktrees/` pinned months back and hidden from `git status` by
...
hit under `.Codex/worktrees/` as noise. Check the worktree for uncommitted work before suggesting
