...
open class DatabaseModule {
    ...
    internal val migration36to37 = object : Migration(36, 37) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `${TABLE_AUTOISF_VALUES}` ADD COLUMN `acceIsfWeight` DOUBLE NOT NULL DEFAULT 0.0")
            db.execSQL("ALTER TABLE `${TABLE_AUTOISF_VALUES}` ADD COLUMN `fslCalSlope` DOUBLE NOT NULL DEFAULT 0.0")
            dropCustomIndexes(db)
        }
    }
